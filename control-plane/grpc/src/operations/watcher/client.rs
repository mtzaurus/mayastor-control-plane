use crate::{
    ...
    watcher::{get_watches_reply, watcher_grpc_client::WatcherGrpcClient},
};
use common_lib::{
...
pub struct WatcherClient {
    inner: Client<WatcherGrpcClient<TracedChannel>>,
}
impl Deref for WatcherClient {
    type Target = Client<WatcherGrpcClient<TracedChannel>>;
...
impl WatcherClient {
    ...
    pub async fn new<O: Into<Option<TimeoutOptions>>>(addr: Uri, opts: O) -> Self {
        let client = Client::new(addr, opts, WatcherGrpcClient::new).await;
        Self { inner: client }
    }
}

#[tonic::async_trait]
impl WatcherOperations for WatcherClient {
    async fn create(
        &self,
        request: &dyn WatcherInfo,
        ctx: Option<Context>,
    ) -> Result<(), ReplyError> {
